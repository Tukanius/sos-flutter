...
class SavePostList extends StatefulWidget {
  final String? name;
  final Filter? filter;
  final double? height;
  final String? type;
  ...
  @override
  State<SavePostList> createState() => _SavePostListState();
}

class _SavePostListState extends State<SavePostList> with AfterLayoutMixin {
  bool loading = true;
  int page = 1;
  int limit = 1000;
  ...
  post(int page, int limit) async {
    ...
    Result res = await PostApi()
        .likedList(ResultArguments(filter: widget.filter, offset: offset));
    setState(() {
      warningPost = res;
      loading = false;
    });
